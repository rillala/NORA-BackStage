...
export default {
    ...
    for (let i = 0; i < files.length; i++) {
      const reader = new FileReader();
      reader.onload = (e) => {
        this.form.images.push(e.target.result); // 将新图片的DataURL添加到数组中
        this.imageFiles.push(files[i]); // 保存文件对象以备上传
      };
      reader.readAsDataURL(files[i]);
        ...
        // 刪除指定的圖片
        removeImage(index) {
    this.form.images.splice(index, 1); // 从数组中删除图片
    // 如果需要，还可以在这里处理后续的删除逻辑，比如更新后端数据
  },
        addColor() {
            if (this.colorInput) {
    ...
    if (this.productData) {
      ...
      // 假设 images 是一个包含图片 URL 的数组
      // 不需要特别处理，因为我们会直接显示这些图片
      // 并允许用户添加或删除图片
    }
  },
...
<template>
    ...
    <img :src="image" alt="商品图片" style="width: 100px; height: auto;">
